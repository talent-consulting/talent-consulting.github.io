# Joining Talent

## Quick links

- Candidate packs at Talent [here](TBD)
- Latest openings here
- The Junior to Mid Level interview [here](#the-junior-to-mid-level-interview)
- The Lead Devleoper interview [here](#the-senior-level-interview)
- The Archiect / Prinicpal interview [here](#the-principal-level-interview)

When you join talent you will be part of a fast paced, hands on team developing against the GDS standards required by 
government on the projects and citizen portals supplied across the UK

# What we are looking for

Most of all someone with a passion for technology and someone who has the right behaviours when working within a team.
We realise that people are different and someone could be really interested in tech but also be interested in developing people, coaching etc

So we are interested in everyone! We do have some basic behaviours we really are looking for and some things we follow as a company, do you:

1. Share your knowledge with your team, helping others through communication of these ideas. 
2. See success as delivery of a project, rather than an individual piece of that overall solution
3. Enjoy the dialogue of coming up with solutions and accept challenges to your ideas

If this is you please read on

# What can I expect from an interview

We don't just rely on cold calling or blanket emailing to get our expertise, we want to invite people to come look and decide whether they like the company ethic, they like the career progression and the work we do.

Word of mouth and networks within the standard channels is one of our main channels for selling the company to prospective talent.

The interview process is important for both you and the company, we want you to have a great experience, so we have detailed the process as a framework, with different roles having different aspects to the discussions.

The process will be as follows:

1. An initial telephone call for no more 20 minutes to discuss you and what you are looking for. At this stage we feel you have the right skill set and behaviours for the roles we need.
2. Within 1 business day we will be contacting you as to how we felt and whether you will be put through to a full interview. We would give constructive feedback should you not make it to the next stage. We would at this stage indicate whether this is a remote based interview or a physical face to face.
3. The next stage would be a technical interview of around 90 minutes with stakeholders in the business. We would ask you what your favoured language is to develop in and tailor the test accordingly. Under some circumstances we would be looking for a specific language and this would be indicated in the phone interview.

# The Junior to mid-level interview

These interviews would be for our Developer and QA staff.

After you have been selected for next stage after the telephone interview you would be invited to join a ~90 minute interview with a format which can be found below

## What you will need

During the face to face interview you will need

1. A development environment, as you will be asked to solve a problem or issue or develop something from scratch
2. A [teams](https://www.microsoft.com/en-gb/microsoft-teams/group-chat-software) account that we can conduct the interview over should this not be an office based interview.


## Attendees

1. Technical level employee
2. HR / business level employee

## The interview

This is a rough itinerary 

| Duration      | Description |
| ----------- | ----------- |
| 10 mins     | Introduction from us and a bit more about the company |
| 20 mins     | An introduction from the candidate and a resume review about aspects of your experience        |
| 40 mins | Technical test, pairing with the technical member of the panel. 30 mins will be given to pairing and 10 minutes to a Q&A |
| 10 mins | A few [STAR](https://www.thebalancecareers.com/what-is-the-star-interview-response-technique-2061629) behavoural questions |
| 10 mins | Candidate questions, and thank you's, with a definitive date for feedback |

### The technical pairing session

Generally it will be a very informal pairing exercise, where we can start to see how the developer works, how they think and how they attack a problem.

We are NOT looking for a 100% code elegant solution with zero issues but are more interested in discussing different aspects of why decisions were made, how things would be improved etc


# The Senior level interview

These interviews would be for our Lead Developer staff.

After you have been selected for next stage after the telephone interview you would be invited to join a ~90 minute interview with a format which can be found below

## What you will need

During the face to face interview you will need

1. A development environment, as you will be asked to solve a problem or issue or develop something from scratch
2. A [teams](https://www.microsoft.com/en-gb/microsoft-teams/group-chat-software) account that we can conduct the interview over should this not be an office based interview.
3. Access to some white boarding tools, which will be advised prior to the interview, draw.io, miro are good candidates


## Attendees

1. Technical level employee
2. HR / business level employee

## The interview

| Duration      | Description |
| ----------- | ----------- |
| 10 mins     | Introduction from us and a bit more about the company |
| 30 mins     | An introduction from the candidate and a resume review about aspects of your experience |
| 40 mins | Technical test, pairing with the technical member of the panel. 30 mins will be given to pairing and 10 minutes to a Q&A |
| 10 mins | A few [STAR](https://www.thebalancecareers.com/what-is-the-star-interview-response-technique-2061629) behavoural questions |
| 10 mins | Candidate questions, and thank you's, with a definitive date for feedback |

### The technical pairing session

Generally it will be a very informal pairing exercise, where we can start to see how the developer works, how they think and how they attack a problem.

We are NOT looking for a 100% code elegant solution with zero issues but are more interested in discussing different aspects of why decisions were made, how things would be improved etc


# The Principal level interview

These interviews would be for our architecture community staff. They will be geared to the discipline being applied for, for example
Security Architect, Cloud architect etc

After you have been selected for next stage after the telephone interview you would be invited to join a ~90 minute interview with a format which can be found below

## What you will need

During the face to face interview you will need

1. A [teams](https://www.microsoft.com/en-gb/microsoft-teams/group-chat-software) account that we can conduct the interview over should this not be an office based interview.
2. Access to some white boarding tools, which will be advised prior to the interview, draw.io, miro are good candidates

## Attendees

1. Technical level employee
2. HR / business level employee

## The interview

| Duration      | Description |
| ----------- | ----------- |
| 10 mins     | Introduction from us and a bit more about the company |
| 10 mins     | An introduction from the candidate and a resume review about aspects of your experience |
| 30 mins | Presentation on a subject of your choosing (15 minutes). Then a Q&A session |
| 10 mins | A few [STAR](https://www.thebalancecareers.com/what-is-the-star-interview-response-technique-2061629) behavoural questions |
| 20 mins | White boarding session, lets design something! |
| 10 mins | Candidate questions, and thank you's, with a definitive date for feedback |

### The presentation

We'd like the candidate to demonstrate their communication skills on a subject relevant to the role. 

A good example of subjects:

- Micro-service architecture
- Synchronous vs Asynchronous on a cloud platform
- Leading a team

### The white boarding session

We will be sending the candidate a problem that we need a solution for. This problem will be designed for the role we currently have,

A security and network architect would get a different problem to go through than a solutions architect

# Next steps

Should you be successful in our second stage interview, we would like you to have a quick chat with our CEO
to talk about your aims and career, nothing too taxing!

The offer would then come through and we would look to start to agree starting dates and start the ball rolling for hardware
preference and other particulars. You can see more about the welcome pack [here](./welcome.md)